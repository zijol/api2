...
```bash
curl -X GET -G "http://west.api.zijol.com/encrypt" 
```

```javascript
const url = new URL("http://west.api.zijol.com/encrypt");

    let params = {
            "words": "Y6swr0YItkzvnFoZ",
        };
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]));
...
```json
{
    "code": "0",
    "msg": "",
    "result": "ev2:DsxAkn5btD6TlL2c3RpVViHpDQOoWrG0Wq3zwBOgnqIn2Wg0K1aDMv118HPAfqv4"
}
```
...
```bash
curl -X GET -G "http://west.api.zijol.com/decrypt" 
```

```javascript
const url = new URL("http://west.api.zijol.com/decrypt");

    let params = {
            "words": "qeh9tqZdz06F7Y8C",
        };
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]));
