...
class Business extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::connection($connection)->create(
            ...
            function (Blueprint $table) {
                ...
                $table->string('headers', 1024)->default('')->comment('请求头Json');
                $table->string('data', 1024)->default('')->comment('请求数据Json');
                $table->timestampTz('next_time')->default(null)->comment('下次的执行时间');
                $table->string('time_periods', 255)->default(0)->comment('AR时间阶梯Json');
    ...
    {
        $connection = config('admin.database.connection') ?: config('database.default');
        Schema::connection($connection)->dropIfExists('member_user');
